...
-- Flexible module loading helper
local function test_module_wrapper(module) require(module) end

-- If the module is found it is loaded and debian_menu_found == True, 
-- otherwise debian_menu_found == Flase
debian_menu_found = pcall(test_module_wrapper, "debian.menu")

-- Default values for user specific confguration
terminal = "xterm"
editor = os.getenv("EDITOR") or "vi"
editor_cmd = terminal .. " -e " .. editor
user_def_launch = function() end
sloppy_focus = False
opacity_focused = 1.0
opacity_unfocused = 0.85
-- Default modkey.
-- Usually, Mod4 is the key with a logo between Control and Alt.
-- If you do not like this or do not have such a key, I suggest you to 
-- remap Mod4 to another key using xmodmap or other tools.
-- However, you can use another modifier like Mod1, but this may interfere 
-- with applications.
modkey = "Mod4"

-- All of the above can be overridden in config_local.lua
config_local_found = pcall(test_module_wrapper, "config_local")

-- Configure the naughty notification library configuration
...
-- modified from https://github.com/vincentbernat/\
...
-- Table of layouts to cover with awful.layout.inc, order matters. The first
...
mytextclock = awful.widget.textclock(" %a %b %d, %H:%M ", 2)
...
--    configurations. An awesome client corresponds to a pane or split 
...
globalkeys = awful.util.table.join(
  ...
  -- User defined launch
  awful.key({ modkey }, "o", user_def_launch
  )
)

--------------------------
-- Managing/modifying tags
--------------------------
...
-- modify_split: Helper function that performs layout modifications.
-- Depending on which layout is currently active, requests for 
-- horizontal or vertical splits call different functions in awful.tag
...
clientkeys = awful.util.table.join(clientkeys,
  ...
  --
  -- Bug: this doesn't work if there are only minimized clients on the screen
  awful.key({ modkey }, "`",
    function ()
      cyclefocus.cycle(1, {modifier="Super_L", 
                           raise_clients=false,
